...
function Home() {
  ...
  const saveGraph = async () => {
    try {
      const response = await axios.post('http://localhost:5000/saveGraph', {
        nodes,
        lines
      });
  ...
  return (
    <Container className="container" maxWidth="xl">
      <Grid container spacing={2} style={{ height: '90vh' }}>
        <Grid item xs={8}>
          <Paper className="node-container" id="node-container" elevation={3} style={{ height: '100%', position: 'relative' }}>
            {nodes.map((node) => (
              <Node
                key={node.id}
                id={node.id}
